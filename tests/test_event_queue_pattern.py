...
class TestEvent(unittest.TestCase):
    def test_event_creation_and_repr(self):
        ...
        self.assertEqual(repr(event), "Event(type='test_event', payload={'key1': 'value1', 'key2': 123})")
...
class TestEventQueue(unittest.TestCase):
    ...
    def test_singleton_instance(self):
        ...
        self.assertIs(eq1, global_event_queue, "Global instance should be the same singleton.")

    def test_queue_and_get_event(self):
        self.assertFalse(global_event_queue.has_events(), "Queue should be initially empty.")
        
        global_event_queue.queue_event("event1", data="foo")
        self.assertTrue(global_event_queue.has_events(), "Queue should have an event after queuing.")
        ...
        self.assertFalse(global_event_queue.has_events(), "Queue should be empty after getting the event.")
        
        self.assertIsNone(global_event_queue.get_event(), "Getting from empty queue should return None.")
    ...
    def test_peek_event(self):
        self.assertIsNone(global_event_queue.peek_event(), "Peeking an empty queue should return None.")
        ...
        self.assertTrue(global_event_queue.has_events(), "Queue should still have event after peeking.")
        event_after_peek = global_event_queue.get_event()
        self.assertIs(event, event_after_peek, "Event retrieved by get should be the same as peeked.")
        
        self.assertFalse(global_event_queue.has_events(), "Queue should be empty after getting the peeked event.")
        self.assertIsNone(global_event_queue.peek_event(), "Peeking an empty queue should return None again.")
